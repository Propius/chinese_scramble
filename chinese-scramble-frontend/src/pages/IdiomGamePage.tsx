...
export const IdiomGamePage: React.FC = () => {
      ...
      if (errorData?.details?.allQuestionsCompleted || errorData?.error?.includes('已完成所有')) {
        ...
        try {
          if (DEFAULT_FEATURE_FLAGS.ENABLE_NO_REPEAT_QUESTIONS) {
            ...
            console.log('[IdiomGamePage] Calling backend restart from handleStart for user:', username);
          ...
          try {
            ...
            if (errorData?.details?.allQuestionsCompleted || errorData?.error?.includes('已完成所有') || errorData?.error?.includes('已经完成所有')) {
              ...
              try {
                ...
                console.error('[IdiomGamePage] Restart also failed:', retryErr);
                setGameResult(null);
                setHint('');
  ...
  const handleRestartQuiz = async () => {
    try {
      console.log('[IdiomGamePage] Restart button clicked');

      // Only call backend restart if no-repeat feature is enabled
      if (DEFAULT_FEATURE_FLAGS.ENABLE_NO_REPEAT_QUESTIONS) {
        const username = usernameUtils.getUsername() || 'Guest';
        console.log('[IdiomGamePage] Calling backend restart for user:', username);
        const response = await apiClient.post(`/api/idiom-game/restart?playerId=${encodeURIComponent(username)}`);
        console.log('[IdiomGamePage] Restart response:', response);
      }

      // Reset state AND clear localStorage
      console.log('[IdiomGamePage] Resetting state and localStorage');
      setQuizCompleted(false);
      setGameResult(null);
      setTotalScore(0);
      setGamesPlayed(0);
      ...
      // Start new game
      console.log('[IdiomGamePage] Starting new game after restart');
      await handleStart();
    } catch (err: any) {
      console.error('[IdiomGamePage] Restart error:', err);
      console.error('[IdiomGamePage] Error details:', err.response?.data);
